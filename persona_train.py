...
def generate_and_tokenize_prompt(data_point, split, tokenizer, prompt_rolegen: str):
    ...
    for i in range(len(data_point["argument"])):
        ...
        if split.find("train") != -1 or split.find("role") != -1:
            prompt_label = prompt_label.format(role_list=data_point["role"][i])
        prompt_inputs.append(prompt_input)
        prompt_labels.append(prompt_label)
    
    inputs = {}
    inputs["input_ids"] = [
        tokenizer.encode(prompt_input + prompt_label if split.find("train") != -1 or split.find("role") != -1 else prompt_input, add_special_tokens=False)
